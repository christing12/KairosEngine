...
namespace Kairos {
	...
	void RootSignature::SetRootParameters(Uint32 numParams, Uint32 offset, RootParameter params[]) {
		KRS_CORE_ASSERT(offset + numParams < m_NumParams, "INVALID");
		for (Uint32 rootIndex = offset; rootIndex < offset + numParams; rootIndex) {
			m_ParamArray[rootIndex] = params[rootIndex - offset];
		}
	}
