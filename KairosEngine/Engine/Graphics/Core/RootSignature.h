...
public:
	...
	void SetEntry(size_t entryIdx, RootParameter& param) {
		m_ParamArray.get()[entryIdx] = param;
	}

	RootParameter& operator[] (size_t entryIdx) {
		KRS_CORE_ASSERT(entryIdx < m_NumParams, "Issue with access root parameters");
		return m_ParamArray.get()[entryIdx];
	}
...
private:
	...
	Scope<RootParameter[]> m_ParamArray;
	Scope<D3D12_STATIC_SAMPLER_DESC[]> m_SamplerArray;
