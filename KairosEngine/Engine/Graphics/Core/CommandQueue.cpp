...
namespace Kairos {
	CommandQueue::CommandQueue(Ref<class RenderDevice> pDevice, D3D12_COMMAND_LIST_TYPE commandType, const std::wstring& debugName)
		: m_Device(pDevice)
		, m_CommandListType(commandType)
		, m_GPUWaitHandle{ ::CreateEvent(nullptr, false, false, nullptr) }
		, m_AllocatorPool(pDevice.get(), commandType, debugName)
	{
